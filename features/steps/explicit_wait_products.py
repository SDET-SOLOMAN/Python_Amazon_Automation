...
SIGN_IN_POPUP = (By.CSS_SELECTOR, "div.nav-signin-tt.nav-flyout")
...
WORD_REGULAR = (By.XPATH, "//span[contains(@class, 'les-item-card__regular-pric')]")
...
def is_regular_product_name(context, expected_word):
    context.driver.wait.until(EC.text_to_be_present_in_element((WORD_REGULAR), expected_word))
    list_of_items = context.driver.find_elements(*WORD_REGULAR)
    for item in list_of_items:
        print(item.text)
        assert expected_word in item.text, f"Expected word {expected_word} is not in item {item}"
    item_description = context.driver.find_elements(*DESC_OF_ITEMS)
    for name in item_description:
        print(name.text)
